...
# epmcoin_hash
RUN git clone https://github.com/epmcoinpay/epmcoin_hash
RUN cd epmcoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} epmcoin
RUN useradd -u ${USER_ID} -g epmcoin -s /bin/bash -m -d /epmcoin epmcoin
...
RUN mkdir /epmcoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /epmcoin-src && \
