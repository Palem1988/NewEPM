...
const char * const BITCOIN_CONF_FILENAME = "epmcoin.conf";
const char * const BITCOIN_PID_FILENAME = "epmcoin.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "epmcoin" is a composite category enabling all EPMCoin-related debug output
                if(ptrCategory->count(std::string("epmcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EPMCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EPMCoinCore
    // Mac: ~/Library/Application Support/EPMCoinCore
    // Unix: ~/.epmcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EPMCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EPMCoin";
#else
    // Unix
    return pathRet / ".EPMCoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty epmcoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override epmcoin.conf
...
{
    std::string strCopyrightHolders = strPrefix + strprintf(" %u ", 2019) + strprintf(_(COPYRIGHT_HOLDERS), _(COPYRIGHT_HOLDERS_SUBSTITUTION));

    // Check for untranslated substitution to make sure Bitcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitcoin Core") == std::string::npos) {
        strCopyrightHolders = strPrefix + strprintf(" %u-%u ", 2009, nEndYear) + "The Bitcoin Core developers"+"\n" + strPrefix + strprintf(" %u-%u ", 2018, nEndYear) + "The PAC Core developers" + "\n" + strCopyrightHolders;
    }
