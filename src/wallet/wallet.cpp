...
bool CWallet::AbandonTransaction(const uint256& hashTx)
{
    ...
    if (origtx.GetDepthInMainChain() > 0 || origtx.InMempool() || origtx.IsLockedByInstaEPM()) {
        return false;
    }
...
void CWallet::ReacceptWalletTransactions()
{
    ...
    BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
    {
        ...
        if (!wtx.IsCoinBase() && !wtx.IsCoinStake() && nDepth < 0 && !wtx.IsLockedByInstaEPM() && !wtx.isAbandoned()) {
...
CAmount CWallet::GetUnconfirmedBalance() const
{
    CAmount nTotal = 0;
    {
        ...
        {
            const CWalletTx* pcoin = &(*it).second;
            if (!pcoin->IsTrusted() && pcoin->GetDepthInMainChain() == 0 && !pcoin->IsLockedByInstaEPM() && pcoin->InMempool())
                nTotal += pcoin->GetAvailableCredit();
        }
    }
    return nTotal;
                                  ...
                                  std::vector<char>& vfBest, CAmount& nBest, bool fUseInstantSend = false, int iterations = 1000)
{
    ...
    if (!llmq::IsOldInstantSendEnabled()) {
        // The new system does not require special handling for InstaEPM as this is all done in CInstantSendManager.
                                 ...
                                 std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, AvailableCoinsType nCoinType, bool fUseInstantSend) const
{
    ...
    for (unsigned int tryDenom = tryDenomStart; tryDenom < 2; tryDenom++)
    {
        ...
        BOOST_FOREACH(const COutput &output, vCoins)
        {
            ...
            bool fLockedByIS = pcoin->IsLockedByInstaEPM();
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (!SelectCoins(vAvailableCoins, nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseInstantSend))
                {
                    if (nCoinType == ONLY_NONDENOMINATED) {
                        ...
                        if (fUseInstantSend) {
                            ...
                            strFailReason += " " + strprintf(_("InstaEPM requires inputs with at least %d confirmations, you might need to wait a few minutes and try again."), nInstantSendConfirmationsRequired);
                        }
                    }
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstaEPM doesn't support sending values that high yet. Transactions are currently limited to %1 EPM."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-epmcoin-address
                        CScript scriptChange;
                        ...
                        if (nSubtractFeeFromAmount > 0 && newTxOut.IsDust(MinRelayFee()))
                        {
                            ...
                            for (unsigned int i = 0; i < vecSend.size(); i++) // subtract from first recipient
                            {
                                if (vecSend[i].fSubtractFeeFromAmount)
                                {
                                    ...
                                    if (txNew.vout[i].IsDust(MinRelayFee()))
                                    {
...
CAmount CWallet::GetMinimumFee(unsigned int nTxBytes, unsigned int nConfirmTarget, const CTxMemPool& pool, CAmount targetFee)
{
    ...
    // prevent user from paying a fee below CFeeRate::MinRelayFee() or CFeeRate::MinTxFee()
...
std::map<CTxDestination, CAmount> CWallet::GetAddressBalances()
{
...

    {
        ...
        BOOST_FOREACH(PAIRTYPE(uint256, CWalletTx) walletEntry, mapWallet)
        {
            ...
            int nDepth = pcoin->GetDepthInMainChain();
            if ((nDepth < (pcoin->IsFromMe(ISMINE_ALL) ? 0 : 1)) && !pcoin->IsLockedByInstaEPM())
                continue;
...
CAmount CWallet::GetAccountBalance(CWalletDB& walletdb, const std::string& strAccount, int nMinDepth, const isminefilter& filter, bool fAddLocked)
{
    ...
    for (std::map<uint256, CWalletTx>::iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
    {
        ...
        if (nReceived != 0 && ((wtx.GetDepthInMainChain() >= nMinDepth) || (fAddLocked && wtx.IsLockedByInstaEPM())))
            nBalance += nReceived;
        nBalance -= nSent + nFee;
    }
    ...
    if (IsArgSet("-paytxfee"))
    {
        ...
        if (MinTxFee() < MinRelayFee())
        {
            return InitError(strprintf(_("Invalid amount for -paytxfee=<amount>: '%s' (must be at least %s)"),
                                       GetArg("-paytxfee", ""), MinRelayFee().ToString()));
        }
    }
