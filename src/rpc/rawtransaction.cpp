...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the epmcoin address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwG\"   (string) EPMCoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue sendrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "3. instantsend    (boolean, optional, default=false) Use InstaEPM to send this transaction\n"
    ...
    bool fInstaEPM = false;
    if (request.params.size() > 2)
        fInstaEPM = request.params[2].get_bool();
    ...
    if (!fHaveMempool && !fHaveChain) {
        ...
        if (fInstaEPM && !instantsend.ProcessTxLockRequest(*tx, *g_connman)) {
            throw JSONRPCError(RPC_TRANSACTION_ERROR, "Not a valid InstaEPM transaction, see debug.log for more info");
        }
