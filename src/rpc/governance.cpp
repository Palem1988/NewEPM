...
void gobject_prepare_help(CWallet* const pwallet)
{
    throw std::runtime_error(
                ...
                "5. use-IS        (boolean, optional, default=false) InstaEPM lock the collateral, only requiring one chain confirmation\n"
...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 EPM irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on epmcoincentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in epmcoin.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("epmcoin.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in epmcoin.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* EPMCoin features */
    { "epmcoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "epmcoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "epmcoin",               "gobject",                &gobject,                true,  {} },
    { "epmcoin",               "voteraw",                &voteraw,                true,  {} },
