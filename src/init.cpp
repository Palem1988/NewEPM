...
#include "config/epmcoin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    if (!fLiteMode && !fRPCInWarmup) {
        ...
        {
            CFlatDB<CInstantSend> flatdb5("instantsend.dat", "magicInstaEPMCache");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-zmqpubhashtxlock=<address>", _("Enable publish hash transaction (locked via InstaEPM) in <address>"));
    ...
    strUsage += HelpMessageOpt("-zmqpubhashinstantsenddoublespend=<address>", _("Enable publish transaction hashes of attempted InstaEPM double spend in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawblock=<address>", _("Enable publish raw block in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtx=<address>", _("Enable publish raw transaction in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtxlock=<address>", _("Enable publish raw transaction (locked via InstaEPM) in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawinstantsenddoublespend=<address>", _("Enable publish raw transactions of attempted InstaEPM double spend in <address>"));
#endif
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                                  "epmcoin (or specifically: chainlocks, gobject, instantsend, keepass, llmq, llmq-dkg, llmq-sigs, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode", strprintf(_("Disable all EPMCoin specific functionality (Masternodes, PrivateSend, InstaEPM, Governance) (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-sporkaddr=<epmcoinaddress>", strprintf(_("Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.")));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N EPM anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
    ...
    strUsage += HelpMessageGroup(_("InstaEPM options:"));
    strUsage += HelpMessageOpt("-enableinstantsend", strprintf(_("Enable InstaEPM, show confirmations for locked transactions (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-instantsendnotify=<cmd>", _("Execute command when a wallet InstaEPM transaction is successfully locked (%s in cmd is replaced by TxID)"));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/EPMCoinOfficial/EPM>";
    const std::string URL_WEBSITE = "<https://epmcoin.io>";
...
/** Sanity checks
 *  Ensure that EPMCoin is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("EPMCoin Core version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single EPMCoin Core process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    // lite mode disables all EPMCoin-specific functionality
    fLiteMode = GetBoolArg("-litemode", false);
    LogPrintf("fLiteMode %d\n", fLiteMode);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, all EPMCoin-specific functionality is disabled."));
    }

    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/EPMCoinOfficial/EPM/pull/1817 and https://github.com/EPMCoinOfficial/EPM/pull/1743
    ...
    if (!fIgnoreCacheFiles) {
        ...
        if(!flatdb3.Load(governance)) {
            return InitError(_("Failed to load cache from") + "\n" + (pathDB / strDBName).string());
        }
        ...
        if(fEnableInstantSend)
        {
            ...
            uiInterface.InitMessage(_("Loading InstaEPM data cache..."));
            CFlatDB<CInstantSend> flatdb5(strDBName, "magicInstaEPMCache");
    ...
    // ********************************************************* Step 10d: schedule EPMCoin-specific tasks
