    ...
    transactionView(0), abandonAction(0), columnResizingFixer(0)
{
    ...
    instantsendWidget->addItem(tr("All"), TransactionFilterProxy::InstaEPMFilter_All);
    instantsendWidget->addItem(tr("Locked by InstaEPM"), TransactionFilterProxy::InstaEPMFilter_Yes);
    instantsendWidget->addItem(tr("Not locked by InstaEPM"), TransactionFilterProxy::InstaEPMFilter_No);
    ...
    connect(instantsendWidget, SIGNAL(activated(int)), this, SLOT(chooseInstaEPM(int)));
...
void TransactionView::setModel(WalletModel *_model)
{
    ...
    if(_model)
    {
        ...
        transactionView->setColumnWidth(TransactionTableModel::InstaEPM, INSTANTSEND_COLUMN_WIDTH);
...
void TransactionView::chooseInstaEPM(int idx)
{
    if(!transactionProxyModel)
        return;
    transactionProxyModel->setInstaEPMFilter(
        (TransactionFilterProxy::InstaEPMFilter)instantsendWidget->itemData(idx).toInt());
}
...
void TransactionView::showAddressQRCode()
{
    ...
    dialog->setInfo(tr("QR code"), "epmcoin:"+strAddress, "", strAddress);
    dialog->show();
}
