...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered epmcoin address / amount:
    ...
    if(recipients[0].fUseInstantSend && IsOldInstantSendEnabled() && total > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
        Q_EMIT message(tr("Send Coins"), tr("InstaEPM doesn't support sending values that high yet. Transactions are currently limited to %1 EPM.").arg(sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)),
    ...
    if(recipients[0].fUseInstantSend && IsOldInstantSendEnabled()) {
        ...
        if(nVinSize > CTxLockRequest::WARN_MANY_INPUTS) {
            Q_EMIT message(tr("Send Coins"), tr("Used way too many inputs (>%1) for this InstaEPM transaction, fees could be huge.").arg(CTxLockRequest::WARN_MANY_INPUTS),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal epm:URI (epm:XyZ...?message=example)
            {
...
bool WalletModel::transactionCanBeAbandoned(uint256 hash) const
{
    ...
    if (!wtx || wtx->isAbandoned() || wtx->GetDepthInMainChain() > 0 || wtx->IsLockedByInstaEPM() || wtx->InMempool())
        return false;
    return true;
}
