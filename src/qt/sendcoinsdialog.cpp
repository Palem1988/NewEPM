    ...
    platformStyle(_platformStyle)
{
    ...
    if(fLiteMode) {
        ...
        ui->checkUseInstaEPM->setVisible(false);
        ...
        ui->checkUseInstaEPM->setChecked(fUseInstantSend);
    ...
    connect(ui->checkUseInstaEPM, SIGNAL(stateChanged ( int )), this, SLOT(updateInstaEPM()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if(model->IsOldInstantSendEnabled() && ui->checkUseInstaEPM->isChecked()) {
        strFunds += " ";
        strFunds += tr("and InstaEPM");
    }

    for (SendCoinsRecipient& rcp : recipients) {
        rcp.inputType = ui->checkUsePrivateSend->isChecked() ? ONLY_DENOMINATED : ALL_COINS;
        rcp.fUseInstantSend = model->IsOldInstantSendEnabled() && ui->checkUseInstaEPM->isChecked();
    }
...
void SendCoinsDialog::updateInstaEPM()
{
    QSettings settings;
    settings.setValue("bUseInstantX", ui->checkUseInstaEPM->isChecked());
    CoinControlDialog::coinControl->fUseInstantSend = model->IsOldInstantSendEnabled() && ui->checkUseInstaEPM->isChecked();
    coinControlUpdateLabels();
}
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid EPMCoin address"));
        }
        else // Valid address
        {
            CKeyID keyid;
