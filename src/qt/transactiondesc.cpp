...
QString TransactionDesc::FormatTxStatus(const CWalletTx& wtx)
{
    ...
    else
    {
        ...
        if (wtx.IsLockedByLLMQInstaEPM()) {
            strTxStatus += " (" + tr("verified via LLMQ based InstaEPM") + ")";
            return strTxStatus;
        }
        ...
        if(instantsend.IsLockedInstantSendTransaction(wtx.GetHash())) {
            strTxStatus += tr("verified via InstaEPM");
        } else if(!instantsend.IsTxLockCandidateTimedOut(wtx.GetHash())) {
            strTxStatus += tr("InstaEPM verification in progress - %1 of %2 signatures").arg(nSignatures).arg(nSignaturesMax);
        } else {
            strTxStatus += tr("InstaEPM verification failed");
        }
        strTxStatus += ")";
...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit)
{
    ...
    // Message from normal epm:URI (epm:XyZ...?message=example)
