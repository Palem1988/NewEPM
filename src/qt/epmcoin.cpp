...
{
    return QCoreApplication::translate("epmcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in epmcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in epmcoin.qrc)
...
/** Class encapsulating EPMCoin Core startup and shutdown.
...
/** Main EPMCoin application object */
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. EPMCoin Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(epmcoin);
    Q_INIT_RESOURCE(epmcoin_locale);
    ...
    /// 6. Determine availability of data directory and parse epmcoin.conf
    ...
    // epm: links repeatedly have their payment requests routed to this process:
