...
bool CInstantSendManager::ProcessTx(const CTransaction& tx, bool allowReSigning, const Consensus::Params& params)
{
    ...
    auto llmqType = params.llmqForInstaEPM;
    ...
    // Ignore any InstaEPM messages until blockchain is synced
...
void CInstantSendManager::HandleNewRecoveredSig(const CRecoveredSig& recoveredSig)
{
    ...
    auto llmqType = Params().GetConsensus().llmqForInstaEPM;
    if (llmqType == Consensus::LLMQ_NONE) {
        return;
    }
    ...
    std::unordered_map<uint256, CInstantSendLockPtr> removeISLocks;
    {
        ...
        for (auto& p : removeISLocks) {
            ...
            for (auto& in : islock->inputs) {
                ...
                quorumSigningManager->RemoveRecoveredSig(consensusParams.llmqForInstaEPM, inputRequestId);
            }

            // same as in the loop
            quorumSigningManager->RemoveRecoveredSig(consensusParams.llmqForInstaEPM, islock->GetRequestId());
        }
