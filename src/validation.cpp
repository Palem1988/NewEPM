...
#if defined(NDEBUG)
# error "EPMCoin Core cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "EPMCoin Signed Message:\n";
                              ...
                              const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    BOOST_FOREACH(const CTxIn &txin, tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in EPMCoin
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
	...
	else if (nHeight <= Params().GetConsensus().nLastPoWBlock && nHeight > 0) {
		nSubsidy = 100000 * COIN;
	}
	else if (nHeight <= 1200200 && nHeight > Params().GetConsensus().nLastPoWBlock) {
		nSubsidy = 50 * COIN;
	}
	else {
		nSubsidy = 0 * COIN;
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
	...
	else if (nHeight <= 180200 && nHeight > Params().GetConsensus().nLastPoWBlock) {
		ret = blockValue * 0.96;  //96%;
	}
	else if (nHeight <= 302000 && nHeight > 180200) {
		ret = blockValue * 0.9;  //90%;
	}
	else if (nHeight <= 452000 && nHeight > 302000) {
		ret = blockValue * 0.86;  //86%;
	}
	else if (nHeight <= 702000 && nHeight > 452000) {
		ret = blockValue * 0.82;  //82%;
	}
	else if (nHeight <= 1000000 && nHeight > 702000) {
		ret = blockValue * 0.8;  //80%;
	}
	else if (nHeight <= 1200200 && nHeight > 1000000) {
		ret = blockValue * 0.78;  //78%;
	}
	else {
		ret = 0 * COIN;
	}
...
static int64_t nTimeEPMCoinSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // EPM : CHECK TRANSACTIONS FOR INSTANTSEND
        ...
        for (const auto& tx : block.vtx) {
            ...
            for (const auto& txin : tx->vin) {
                ...
                if (instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx->GetHash()) {
                    ...
                    return state.DoS(10, error("ConnectBlock(EPM): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), hashLocked.ToString()),
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
                return state.DoS(10, error("ConnectBlock(EPM): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
        LogPrintf("ConnectBlock(EPM): spork is off, skipping transaction locking checks\n");
    }
    ...
    // EPM : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(EPM): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (FullDIP0003Mode() || pindex->nHeight == Params().GetConsensus().nGenerationHeight) {
       if (!IsBlockPayeeValid(*block.vtx[isProofOfStake], pindex->nHeight, blockReward - nFees)) {
           mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
           return state.DoS(0, error("ConnectBlock(EPM): couldn't find masternode or superblock payments"), REJECT_INVALID, "bad-cb-payee");
       }
    }
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(EPM): ProcessSpecialTxsInBlock for block failed with %s", FormatStateMessage(state));
    }
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeEPMCoinSpecific += nTime5 - nTime4;
    LogPrint("bench", "    - EPMCoin specific: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeEPMCoinSpecific * 0.000001);
