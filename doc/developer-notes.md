...
EPMCoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 1504 for connections and services them.
...
However, in open source software such as EPMCoin Core, where everyone uses
...
pay attention to for reviewers of EPMCoin Core code.

General EPMCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: EPMCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against EPMCoin Core so that they can be integrated
