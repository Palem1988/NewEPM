...
// which are part of the EPMCoin repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="epmcoin-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /epmcoin-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "epmcoinpay-epmcoin/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from epmcoinpay-epmcoin/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'epmcoinpay-epmcoin/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from epmcoinpay-epmcoin/develop"
            }
          }

          if (hasCache) {
            sh "cd /epmcoin-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /epmcoin-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /epmcoin-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /epmcoin-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /epmcoin-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /epmcoin-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /epmcoin-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
